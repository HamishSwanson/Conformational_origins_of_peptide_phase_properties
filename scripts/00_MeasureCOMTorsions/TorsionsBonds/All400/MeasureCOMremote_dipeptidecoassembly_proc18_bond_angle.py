...
class DihedralAnalysis:
    ...
    def init_data(self):
        ...
        self.gro_files = sorted([file for file in gro_files if 'dry' in str(file)])
        self.xtc_files = sorted([file for file in xtc_files if 'dry' in str(file)])
    ...
    def process_file_pair(self, file_pair):
        ...
        self.molNum         = 1                                                    ## single molecule
        self.species        = (str(gro_file).split('/')[-1]).split('_')[0]         ## e.g., AA_dry.gro [AA,dry,gro]
        self.peptide_length = int(len(self.species))
        gro_dir             = gro_file.parent
        local_directory     = f'{self.species[0]}X/{self.species}'
...
if __name__ == "__main__":
    ...
    BuildDirs()
    path        = f'path/to/concseries/data/in/zenodo/dataset/Single_Trajectories/Traj/'
